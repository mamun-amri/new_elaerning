...
class Materi extends MY_Controller
{
    ...
    {
        // require_once site_url() . 'assets/vendor_mpdf/autoload.php';
        $tgl = date('Y-m-d', time());
        $query = "SELECT * FROM materi WHERE tgl_posting ='$tgl'";
        // $query = "SELECT * FROM materi  WHERE tgl_posting ='2021-07-10';
    ...
    {
        $before = date('Y-m-d', (time() - 60 * 60 * 24 * 7));
        $now = date('Y-m-d', time());
        $query = "SELECT * FROM materi WHERE tgl_posting BETWEEN '$before' AND '$now'";
        // $query = "SELECT * FROM materi  WHERE tgl_posting ='2021-07-10';
        $row = $this->db->query($query)->result();
    ...
    {
        $before = date('Y-m-d', (time() - 60 * 60 * 24 * 30));
        $now = date('Y-m-d', time());
        $query = "SELECT * FROM materi WHERE tgl_posting BETWEEN '$before' AND '$now'";
        // $query = "SELECT * FROM materi  WHERE tgl_posting ='2021-07-10';
        $row = $this->db->query($query)->result();
